...
(function ($) {
  ...
  * Define Drupal behaviours
  */
  Drupal.behaviors.filter_listing = {
    attach: function (context, settings) {
      ...
      const filter = {
        init() {
          filter.searchFormByClicking();
          filter.searchFormByEnter();
        },

        getRadioButton() {
          let radioButton = null, userSelection = null;
          radioButton = document.querySelectorAll('input[name="optradio"]');
          ...
          return (userSelection !== null) ? userSelection.value : null;
        },

        getSelectButton() {
          let selectButton = null, userSelection = null;
          selectButton = document.querySelector('#filter_select');
          if (selectButton === null) {
            return;
          }
          userSelection = selectButton.options[selectButton?.selectedIndex].value
          return (userSelection !== null) ? userSelection : null;
        },

        getInputValue() {
          let inputValue = null;
          inputValue = document.querySelector('#filterList');
          return inputValue.value;
        },

        isRadioButtonHidden() {
          let radioButton = null, userSelectionIsHidden = null, style = null;
          radioButton = document.querySelector('.radio-container');
          style = window.getComputedStyle(radioButton);
          userSelectionIsHidden = (style.display === 'none')
          return userSelectionIsHidden;
        },

        searchFormByClicking() {
          document.querySelector('#filter-btn').addEventListener('click', function() {
            let radioChecked = null, selectedButton = null, inputValue = null, isRadioHidden = null;
            radioChecked = filter.getRadioButton();
            selectedButton = filter.getSelectButton();
            inputValue = filter.getInputValue();
            isRadioHidden = filter.isRadioButtonHidden();

            if (radioChecked !== null && !isRadioHidden) {
              filter.goToPage(radioChecked, inputValue);
            } else if (selectedButton  !== null && inputValue  !== null & isRadioHidden) {
              filter.goToPage(selectedButton, inputValue);
            }
          });
        },

        searchFormByEnter() {
          document.querySelector('#filterList').addEventListener('keypress', function(e) {
            if (e.key === 'Enter') {
              let radioChecked = null, selectedButton = null, inputValue = null, isRadioHidden = null;
              radioChecked = filter.getRadioButton();
              selectedButton = filter.getSelectButton();
        ...
        goToPage(url, sorting_keyword) {
          let filterType = null;
          filterType = document.querySelector('#filter');
          optargs = $("#filterList").attr("data-optargs");

          if (filterType?.dataset?.type === "combine-filter") {
            document.location.href = url + "=" + sorting_keyword;
          } else {
            if (typeof optargs !== 'undefined' && optargs.length) {
              optargs = optargs.replace("{s}", sorting_keyword);
              document.location.href = url + optargs;
            } else {
              document.location.href = url + '?sort_keyword=' + sorting_keyword;
            }
          }
        }
      }
