...
(function ($) {
  ...
  * Define Drupal behaviours
  */
  Drupal.behaviors.phd_projects_list = {
    attach: function (context, settings) {
      $('form[data-drupal-selector=views-exposed-form-phd-projects-list-block-1] .form-item label').each(function() {
        $(this).addClass('hidden');
      });
      $('form[data-drupal-selector=views-exposed-form-phd-projects-list-block-1] .form-item input').each(function() {
        $(this).removeClass('rounded shadow py-2 px-3 text-dove');
        $(this).addClass('shadow-card p-6 relative placeholder:text-dove text-actualblack');
      });
      $('form[data-drupal-selector=views-exposed-form-phd-projects-list-block-1] .form-item select').each(function() {
        $(this).removeClass('rounded shadow py-2 pl-3');
        $(this).addClass('shadow-card p-6 text-dove style-select');
      });
      $('form[data-drupal-selector=views-exposed-form-phd-projects-list-block-1] .form-item .centred-icon-container').each(function() {
        $(this).removeClass('-ml-12');
        $(this).addClass('bg-endeavour text-white h-17 w-17 -ml-17 absolute');
      });
      $('form[data-drupal-selector=views-exposed-form-phd-projects-list-block-1] div[data-drupal-selector=edit-actions]').each(function() {
        $(this).addClass('clear-both');
      });
      $('#submitter').on('mouseup touchend keyup', function(e) {
        $('form[data-drupal-selector=views-exposed-form-phd-projects-list-block-1] div[data-drupal-selector=edit-actions] input.button').click();
      });

      // Submit hidden submit button
      $("#submitter").click(function(e){
        $(this).siblings('form').find('input.form-submit').trigger('click'); // Submit, handles AJAX submission.
      });
    }
  };
...
/**
 * @file
 * Controls the behaviour of the fake select menus.
 */
    ...
    attach: function (context, settings) {
      ...
      */
      ...
      function setInitialFocus() {
        ...
        // Build list of fake-selector targets, ie hidden fields that are
        // updated when a fake-select option is clicked on.
        $('.fake_select_container').each(function(index) {
          target_id = $(this).attr('rel');
          fake_selects.push($('#' + target_id).attr('name'));
        });

        // Get any url parameters and, if any are populated then move focus
        // to the form.
        const urlParams = new URLSearchParams(window.location.search);
        let url_params_keys = urlParams.keys();
        for (const key of url_params_keys) {
          var qry_val = urlParams.get(key);
          if (fake_selects.includes(key) && qry_val.length) {
            $('.fs-selected[rel="' + key + '"]').focus();
            break;
          }
        }
      }
      ...
      function functionality() {
        // Declare variables.
        var current_fake_select = '';
        var current_fs_target = '';
        var this_fs_selector_value = '';

        // Set the val() of `edit-submit-*` to a single space.
        $('input[data-drupal-selector^="edit-submit-"]').val(' ');

        // Remove `relative` class from the submit button.
        $('input[data-drupal-selector^="edit-submit-"]').removeClass('relative');

        // Show the correct .fs-selector.
        $('.fake_select_container').each(function(index) {
          target_id = $(this).attr('rel');
          latest_value = $('[data-drupal-selector="' + target_id + '"]').val();
          this_container_id = $(this).attr('id');
          // Search the fake select elements looking for the right val() and grab the text from it.
          $(this).find('.fs-selector').each(function(i) {
            if ($(this).attr('rel') == latest_value) {
              $('#' + this_container_id + ' .fs-selected').text($(this).text());
              var fs_selected_aria_label = $('#' + this_container_id + ' .fs-selected').attr('aria-label');
              let aria = fs_selected_aria_label.replace(/'[^']+'/g, "'" + $(this).text() + "'");
              $('#' + this_container_id + ' .fs-selected').attr('aria-label', aria);
            }
          });
        });
        ...
        $(document).on("mouseup touchend keyup", function(event){
          if ((event.type == 'keyup' && (event.which == 13 || event.which == 32)) || event.type == 'mouseup' || event.type == 'touchup') {
          // if ((event.type == 'mouseup' || event.type == 'touchup')) {
            // If it's outside of .fs-selected or .fs_selector
            if (!$(event.target).hasClass('fs-selector') && !$(event.target).hasClass('fs-selected')) {
              // Close all fake-select menus
              $('.fs-selector-container').addClass('hidden').attr('aria-expanded', 'false');
              $('.form-select').removeClass('focus');
              $('.centred-icon-container svg').removeClass('rotated');
              $('.fs-selector-padding .fs-selector').removeClass('sl-focused').attr('aria-selected', false);
              current_fake_select = '';
            }
            else if ($(event.target).hasClass('fs-selected')) {
              // If there's a menu already open...
              if (current_fake_select > '') {
                $('#' + current_fake_select + ' .fs-selector-container').addClass('hidden').attr('aria-expanded', 'false');
              }
              if (current_fake_select == $(event.target).parents('.fake_select_container').attr('id')) {
                $(event.target).siblings('.fs-selector-container').addClass('hidden').attr('aria-expanded', 'false');
                $(event.target).parents('.form-select').removeClass('focus');
                $(event.target).parents('.fake_select_container').find('.centred-icon-container svg').removeClass('rotated');
                $('.fs-selector-padding .fs-selector').removeClass('sl-focused').attr('aria-selected', false);
                current_fake_select = '';
              }
              else {
                current_fake_select = $(event.target).parents('.fake_select_container').attr('id');
                $(event.target).siblings('.fs-selector-container').removeClass('hidden').attr('aria-expanded', 'true');
                $(event.target).parents('.form-select').addClass('focus');
                $(event.target).parents('.fake_select_container').find('.centred-icon-container svg').addClass('rotated');
              }
            }
            else if ($(event.target).hasClass('fs-selector')) {
              current_fs_target = $(event.target).parents('.fake_select_container').attr('rel');
              this_fs_selector_value = $(event.target).attr('rel');
              $('[data-drupal-selector="' + current_fs_target + '"]').val(this_fs_selector_value).change();
              $(event.target).parents('.fs-selector-container').siblings('.fs-selected').text($(event.target).text());
              $(event.target).parents('.fs-selector-container').addClass('hidden').attr('aria-expanded', 'false');
              $(event.target).parents('.form-select').removeClass('focus');
              $(event.target).parents('.fake_select_container').find('.centred-icon-container svg').removeClass('rotated');
              $('.fs-selector-padding .fs-selector').removeClass('sl-focused').attr('aria-selected', false);
              current_fake_select = '';
              $(event.target).parents('form').find('div[data-drupal-selector=edit-actions] input.button[type="submit"]').click();
            }
          }
        });
      }
...
(function ($) {
   ...
   * Define Drupal behaviours
   */
  Drupal.behaviors.form_accessibility = {
    attach: function (context, settings) {
      ...
      $.fn.Close_All_Fake_Selects = function() {
        // Close all open fake-selects
        $('.fs-selector-container').addClass('hidden');
        $('.form-select').removeClass('focus');
        $('.centred-icon-container svg').removeClass('rotated');
        $('.fs-selector-padding .fs-selector').removeClass('sl-focused').blur();
      };

      $.fn.Scroll_Within_Open_Select = function(scroll_direction) {

        var current_focus = $(':focus');

        // First consider if 'type-1' select is opened
        if (current_focus.find('.selectCustom').length) {
          target_select = current_focus.find('.selectCustom')
          type1_select_options = target_select.find('.selectCustom__options');
          focused_index = type1_select_options.find('.selectCustom__option--hover').index();
          select_count = type1_select_options.find('.selectCustom__option').length;
          focused_index_next = scroll_direction == 'up' ? focused_index : focused_index+2;
          focused_index_next = focused_index_next < 1 ? 1 : focused_index_next;
          focused_index_next = focused_index_next > select_count ? select_count : focused_index_next;
          current_focus.parent().find('.selectCustom__option').removeClass("selectCustom__option--hover");

          next_focused = type1_select_options.find('.selectCustom__option:nth-of-type(' + focused_index_next + ')')
          next_focused.addClass('selectCustom__option--hover')
          SelContainer = type1_select_options;
          $.fn.Handle_Styled_Select_Scroll_Visibility(SelContainer, next_focused, focused_index, scroll_direction);
        }

        // Otherwise, check if a fake select is opened
        else if ($('.fs-selector-container').is(":visible")) {
          target_select = $('.fs-selector-container:visible').first();
          focused_index = target_select.find('.fs-selector-padding .fs-selector.sl-focused').index();
          select_count = target_select.find('.fs-selector-padding .fs-selector').length;
          focused_index_next = scroll_direction == 'up' ? focused_index : focused_index+2;
          focused_index_next = focused_index_next < 1 ? 1 : focused_index_next;
          focused_index_next = focused_index_next > select_count ? select_count : focused_index_next;

          target_select.find('.fs-selector-padding .fs-selector').removeClass('sl-focused').blur();
          next_focused = target_select.find('.fs-selector-padding .fs-selector:nth-of-type(' + focused_index_next + ')')
          next_focused.addClass('sl-focused').focus();
          SelContainer = target_select.find('.fs-selector-padding');
          $.fn.Handle_Styled_Select_Scroll_Visibility(SelContainer, next_focused, focused_index, scroll_direction);
        }
      };

      /**
       * Ensure that when the UP / Down keys are pressed,
       * that the highlighted select option remains visible
       * in its scrollable container
       */
      $.fn.Handle_Styled_Select_Scroll_Visibility = function (SelContainer, next_focused, focused_index, scroll_direction) {
        get_select_focus_state = next_focused.visibleInScroll();

        // Read out selection text on screen reader
        selection_txt = next_focused.text().trim();
        if (selection_txt.length) {
          $.fn.srSpeak('Select option - ' + selection_txt, 'assertive');
        }

        if (!get_select_focus_state.isVisible || get_select_focus_state.visibleHeight < 65) {
          scoll_pos = focused_index < 1 ? 1 : focused_index;
          if (scroll_direction == 'up') {
            SelContainer.animate({
                scrollTop: (Math.round(scoll_pos / 5) * 279.625) - 70
            }, 50);
          }
          else {
            SelContainer.animate({
                scrollTop: Math.round(focused_index / 5) * 350
            }, 50);
          }
        }
      };
       ...
       */
       ...
       $.fn.asscRemoveMiscUneededFormElements = function () {
          // Remove empty legend elements
          if ($('fieldset .fieldset-legend').length) {
              $("fieldset .fieldset-legend").each(function () {
                if (!$(this).text().trim().length && $(this).parent().is("legend")) {
                  $(this).parent().remove();
                }
            });
          }
      ...
      /**
       * Read out selected text via default screen reader, by dynamically creating and updating a hidden div,
       * and reading out the text before destroying the div once completed.
       *
       * @param text The text to be read out, in plain text.
       * @param priority (optional) The priority the screen reader should place this request (the 'aria-live' status),
       *        usually one of 3 options:
       *        "off": default value, the user is not told when the element is updated.
       *        "polite": the screen reader vocalises in a polite manner when the element is updated. This means that if the screen reader was already reading text, it will not be interrupted.
       *        "assertive": the screen reader is interrupted in its reading and tells the user about the update straight away.
       */
      $.fn.srSpeak = function (text, priority) {
      ...
      /*========== End of Functions ==========*/
      if (!isCallAjax()) {
        $.fn.asscRemoveMiscUneededFormElements();
      }

      // Handle 'fake' styled selects keyboard tabbing,
      // If form element in wrapped in a link
      // with class 'acc-control-link-wrapper', and the
      // user presses 'Enter', then simulate a click
      // on the link's immediate child element to open select.
      // Also consider up / down keys for open selects.
      // Tabbing should close an open select.
      // Note there are 2 types of styled selects - one has the
      // '.selectCustom' class and will be known as 'type-1'
      // selects below.
      if ($wind.width() > 767 && $('a.acc-control-link-wrapper').length) {
        $(window).keydown(function (e) {

          var arrow = { tab: 9,  enter: 13, left: 37, up: 38, right: 39, down: 40 };
          ...
          switch (e.which) {
            ...
            case arrow.enter:
              ...
              // Look for 'type-1' select
              if (current_focus.find('.selectCustom').length) {
                type1_select = current_focus.find('.selectCustom');
                if (type1_select.hasClass("selectCustom--active")) {
                  // Before closing the select, first check if the
                  // highlighted option is the current active one;
                  // if not click it and submit the form
                  current_selected_text = current_focus.parent().find('.selectCustom__triggerText').text().trim();
                  current_highlighted_text = current_focus.parent().find('.selectCustom__option--hover').text().trim();
                  if (current_selected_text == current_highlighted_text) {
                    type1_select.removeClass( "selectCustom--active" );
                    type1_select.attr("aria-hidden","true");
                  }
                  else {
                    current_focus.parent().find('.selectCustom__option--hover').trigger("click")
                  }
                }
                else {
                  type1_select.addClass( "selectCustom--active" );
                  type1_select.removeClass("selectCustom__option--hover");
                  type1_select.attr("aria-hidden","false");
                  // Scroll to open select
                  $('html, body').animate({
                    scrollTop: current_focus.offset().top - ($wind.height() * .25)
                  }, 300);
                }
              }
              else if ($('.fs-selector-container').is(":visible") && $('.fs-selector-container .sl-focused').length) {
                // Click highlighted item
                target_select = $('.fs-selector-container:visible').first().find('.fs-selector-padding .fs-selector.sl-focused');
                get_sel_id = target_select.closest('.fake_select_container').attr('rel');
                rel_value = target_select.attr('rel');
                rel_text = target_select.text();
                target_select.closest('.fake_select_container').find('.fs-selected').text(rel_text);
                $('#' + get_sel_id).val(rel_value).change();
                $('#' + get_sel_id).closest('form').find('div[data-drupal-selector=edit-actions] input.button[type="submit"]').click();
                setTimeout( function(){
                  $.fn.Close_All_Fake_Selects();
                }, 500 );
              }
              else {
                // Now handle previous and next tabbing,
                // close all open fake-selects
                if ($('.centred-icon-container svg.rotated')) {
                  $.fn.Close_All_Fake_Selects();
                }

                form_element_has_focus = $('a.acc-control-link-wrapper').is(':focus');
                if (form_element_has_focus) {

                  target_select = current_focus.find('.fs-selector-container');

                  if (target_select.hasClass('hidden')) {
                    $.fn.Close_All_Fake_Selects();
                    target_select.removeClass('hidden');
                    target_select.parents('.form-select').addClass('focus');
                    current_focus.find('.centred-icon-container svg').addClass('rotated');
                    // Scroll to open select
                    $('html, body').animate({
                      scrollTop: target_select.offset().top - ($wind.height() * .25)
                    }, 300);
                    // Give first select item highlight
                    target_select.find('.fs-selector-padding .fs-selector:first-of-type').addClass('sl-focused').focus();
            ...
            //////////////////////
            case arrow.down:
              // First disable browser scroll
              e.preventDefault();
              $.fn.Scroll_Within_Open_Select('down');
              break;
            ...
            case arrow.up:
              // First disable browser scroll
              e.preventDefault();
              $.fn.Scroll_Within_Open_Select('up');
              break;
