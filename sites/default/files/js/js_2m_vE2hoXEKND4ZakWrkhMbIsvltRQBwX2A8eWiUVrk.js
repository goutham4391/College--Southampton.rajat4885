  ...
  */
  Drupal.behaviors.research_search_form = {
    attach: function (context, settings) {
      ...
      // Ensure we run once and once only.
      if (context !== document) {
        return;
      }

      // Initialise statuses.
      $('form[data-drupal-selector=research-search-form] #edit-research-types input.form-radio').each(function() {
      ...
      var research_types_radios = [];
      $('#edit-research-types .form-item').each(function rtr() {
        research_types_radios.push($(this).attr('id'));
      });

      // Tidy up styles.
      $('form[data-drupal-selector=research-search-form] #edit-research-types label').each(function() {
        $(this).removeClass('text-sm mb-1');
        $(this).addClass('lg:text-xl leading-tight');
      });
      $('form[data-drupal-selector=research-search-form] #edit-research-types .form-item.form-type-radio').each(function() {
        $(this).removeClass('mb-4');
      });

      // Handle clicks on each radio-button element.
      $('form[data-drupal-selector=research-search-form] #edit-research-types label').on('mouseup touchend keyup', function(e) {
        if ((e.type == 'keyup' && (e.which == 13 || e.which == 32)) || (e.type == 'mouseup' || e.type == 'touchend')) {
          $('.checked').removeClass('checked').show();
          $(this).parent().addClass('checked');
          $(this).siblings('input.form-radio').prop('checked', true);
          toggleRadios();
        }
      });

      // Handle clicks on the radio-button menu opener.
      $('#radio-opener').on('mouseup touchend keyup', function(e) {
        if ((e.type == 'keyup' && (e.key == 'Space' || e.key == 'Enter')) || (e.type == 'mouseup' || e.type == 'touchend')) {
          toggleRadios();
        }
      });

      // Handle up/down arrow clicks while #edit-research-types is open.
      $(document).on('keyup', function(e) {
        if (e.type == 'keyup' && (e.key == 'ArrowUp' || e.key == 'ArrowDown')) {
          // If focus is on the opener...
          if (document.activeElement.id == 'radio-opener') {
            $('#radio-opener').siblings('.form-item-research-types').first().children('label').focus();
          }

          var forValue = document.activeElement.attributes.for.nodeValue;
          // Check if it's a label for a radio button.
          if (forValue.indexOf('edit-research-types') == 0) {
            if (e.key == 'ArrowDown') {
              if ($('label[for="' + forValue + '"]').parent().attr('id') != research_types_radios[research_types_radios.length -1]) {
                $('label[for=' + forValue + ']').parent().next('.form-item-research-types').children('label').focus();
              }
            }
            if (e.key == 'ArrowUp') {
              if ($('label[for="' + forValue + '"]').parent().attr('id') != research_types_radios[0]) {
                $('label[for=' + forValue + ']').parent().prev('.form-item-research-types').children('label').focus();
              }
            }
          }
        }
      ...
      // Handle clicks on the submitter.
      $('#submitter').on('mouseup touchend keyup', function(e) {
        // Forward key input as click only if its Space or Enter
        if (e.type == 'keyup' && !(e.which == 13 || e.which == 32)) {
          return;
        }

        $('form[data-drupal-selector=research-search-form] input.button[data-drupal-selector=edit-submit]').click();
      });

      // Hide dropdown menu on click outside
      $(document).on('mouseup touchend keyup', function(event){
        if(!$(event.target).closest('#edit-research-types').length){
          if ($('#edit-research-types').hasClass('open')) {
            toggleRadios();
          }
        }
      });
      ...
      function toggleRadios() {
        if ($('#edit-research-types').hasClass('open')) {
          $('#edit-research-types').removeClass('open');
        }
        else {
          $('#edit-research-types').addClass('open');
        }
        $('form[data-drupal-selector=research-search-form] #edit-research-types .form-item.form-type-radio:not(.checked)').toggle();
      }
