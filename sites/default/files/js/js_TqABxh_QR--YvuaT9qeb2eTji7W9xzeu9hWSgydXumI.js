...
(function ($) {
  ...
  * Define Drupal behaviours
  */
  Drupal.behaviors.filter_additional = {
    attach: function (context, settings) {
      ...
      // Filter cards and update count and jump links
      $.fn.hideFilterCards = function (elem, txt) {
        var this_txt = txt.toLowerCase();
        var count_filter = 0;
        var typePlural = $('section.results .counter').attr('data-filter-type-p');
        var typeSingular = $('section.results .counter').attr('data-filter-type-s');
        var alphabet = "";

        $('.views-element-container ' + elem).each(function () {
          var haystack = $(this).attr("data-filter-card").toLowerCase();
          if (this_txt != null && haystack != null && haystack.indexOf(this_txt) >= 0) {
            count_filter ++;
            var first_letter = haystack.trim().charAt(0);
            if (!(alphabet.indexOf(first_letter) >= 0)) {
              alphabet = alphabet + first_letter;
            }
          }
          else {
        ...
        if (count_filter > 1 || count_filter === 0) {
          count_text = count_filter + ' ' + typePlural;
        }
        else {
          count_text = count_filter + ' ' + typeSingular;
        }

        $('section.results .counter').text(count_text);

        if (count_filter < 1) {
          $('div.result.hidden').removeClass('hidden');
        }

        $('nav.scrollbar-block a.control').addClass('text-dove');
        $('nav.scrollbar-block a.control').removeAttr("href");

        for (var i = 0; i < alphabet.length; i++) {
          if ($('nav.scrollbar-block [data-filter-jump="' + alphabet.charAt(i) + '"]').length > 0) {
            $('nav.scrollbar-block [data-filter-jump="' + alphabet.charAt(i) + '"]').removeClass('text-dove');
            $('nav.scrollbar-block [data-filter-jump="' + alphabet.charAt(i) + '"]').attr("href", "#" + alphabet.charAt(i));
          }
        }
      };

      $.fn.saveInputVal = function() {
        var input_val = $('#filterList').val().trim();
        if (input_val.length) {
          $.cookie("filtered_listing_input_val", input_val, {expires: 1});
        }
      };

      $.fn.makeUrlQuery = function(whichRadio) {
        var input_val = $('#filterList').val().trim();
        if (input_val.length) {
          return ('?qry=' + input_val + '&wr=' + whichRadio);
        }
        else {
          return ('?wr=' + whichRadio);
        }
      };

      $.fn.setInputVal = function() {
        if (window.location.href.indexOf('?qry=') >= 0) {
          let searchParams = new URLSearchParams(window.location.search);
          var qry_val = searchParams.get('qry');
          if (qry_val.length) {
            $('#filterList').val(qry_val.trim());
          }
        }
      };
      ...
      $.fn.filterItems = function() {
        if (window.location.href.indexOf('?qry=') >= 0) {
          setTimeout(function () {
            // Do this after delay
            $('#filterList').focus();
            $.fn.hideFilterCards('[data-filter-card]', $('#filterList').val());
          }, 500);
        }
      };

      $.fn.returnFocus = function(whichRadio) {
        // If we have come from this form via a forced reload caused by
        // a radio button...

        // Get the id of the radio button itself...
        let thisRadio = $('#' + whichRadio + ' > input').attr('id');
        // ...and its index...
        let thisRadioIndex = thisRadio.charAt(thisRadio.length - 1);
        $('#' + thisRadio).prop('checked', true);
        $('#' + whichRadio[thisRadioIndex]).focus();
      };

      const queryString = window.location.search;
      const urlParams = new URLSearchParams(queryString);
      if (urlParams.has('wr') && urlParams.get('wr').trim().length > 0) {
        $.fn.returnFocus(urlParams.get('wr'));
      }
      ...
      var base_url = full_url.substring(0, full_url.indexOf("/postgraduate"));
      if (base_url.trim().length == 0) {
        base_url = full_url;
      }
      var opt2_url = base_url + '/postgraduate';

      $.fn.setInputVal();
      $.fn.filterItems();

      $("#filterList").on("keyup change", function(e) {
        // $.fn.saveInputVal();
      });

      // Opt 1 selected
      $(document).on("click", "#filter-opt1", function() {
        window.location.href = base_url + $.fn.makeUrlQuery('filter-opt1');
      });

      // Opt 2 selected
      $(document).on("click", "#filter-opt2", function() {
        window.location.href = opt2_url + $.fn.makeUrlQuery('filter-opt2');
      });
