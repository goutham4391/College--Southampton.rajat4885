...
(function ($) {
   ...
   * Define Drupal behaviours
   */
  Drupal.behaviors.carousel_accessibility = {
    attach: function (context, settings) {
      ...
      // Handle carousel keyboard tabbing,
      // once right button has focus
      // 'Enter' should move carousel to next slide
      if ($wind.width() > 767 && !isCallAjax() && $('div.carousel-container').length) {
        $(window).keydown(function (e) {
          if (e.key === "Enter") {
            carousel_right_btn_focused = $('.acc-carousel-tab').is(':focus');
            if (carousel_right_btn_focused) {
              $(':focus').closest('.swiper-button-next').trigger('click');
            }
          }
       });
      }

      // Handle screen reader keyboard clicks on .block-card
      $(window).on('keyup', function(e) {
        if ((e.which == 13 || e.which == 32) && $( document.activeElement ).hasClass('multi-item-slide')) {
          var clicker = $( document.activeElement ).find('a.card-link-overlay');
...
(function ($) {
  ...
  * Define Drupal behaviours
  */
  Drupal.behaviors.course_finder_form = {
    attach: function (context, settings) {
      ...
      // Initialise statuses.
      $('form[data-drupal-selector=course-finder-form] #edit-course-types input.form-radio').each(function() {
      ...
      // Tidy up styles.
      $('form[data-drupal-selector=course-finder-form] #edit-course-types label').each(function() {
        $(this).removeClass('text-sm mb-1 font-bold');
        $(this).addClass('lg:text-xl leading-tight');
      });
      $('form[data-drupal-selector=course-finder-form] #edit-course-types .form-item.form-type-radio, .form-item-course-finder-form-name-key').each(function() {
        $(this).removeClass('mb-4');
      });

      // Handle clicks on each radio-button element.
      $('form[data-drupal-selector=course-finder-form] #edit-course-types label').on('mouseup touchend keyup', function(e) {
        if ((e.type == 'keyup' && (e.which == 13 || e.which == 32)) || e.type == 'mouseup' || e.type == 'touchend') {
          ...
          // Change text of cff_link_courses link.
          if ($(this).siblings('input.form-radio').val() == 'postgraduateresearch') {
            $('#cff_link_courses').text('PhD projects');
            $('#cff_link_browse').text('PhD topics');
          }
          else {
            $('#cff_link_courses').text('Courses A to Z');
            $('#cff_link_browse').text('Browse by subject area');
          }
          toggleRadios();
        }
      });
      ...
      $('#submitter').on('mouseup touchend keyup', function(e) {
        if ((e.type == 'keyup' && (e.which == 13 || e.which == 32)) || e.type == 'mouseup' || e.type == 'touchend') {
          $('form[data-drupal-selector=course-finder-form] input.button[data-drupal-selector=edit-submit]').click();
        }
      });

      // Handle clicks on one of the cff_links.
      $('.cff_link').on('mouseup touchend keyup', function(e) {
        e.preventDefault();
        if ((e.type == 'keyup' && (e.which == 13 || e.which == 32)) || e.type == 'mouseup' || e.type == 'touchend') {
          if ($(this).attr('id') == 'cff_link_courses') {
            switch($('#edit-course-types input.form-radio:checked').val()) {
              case 'undergraduate':
                window.location.href = '/courses';
                break;
              case 'postgraduate':
                window.location.href = '/courses/postgraduate';
                break;
              case 'postgraduateresearch':
                window.location.href = '/study/postgraduate-research/projects';
                break;
              default:
                // do nothing
            }
            ...
            switch($('#edit-course-types input.form-radio:checked').val()) {
              ...
              case 'postgraduate':
                ...
                break;
              case 'postgraduateresearch':
                window.location.href = '/study/postgraduate-research';
                break;
              default:
                // do nothing
            }
      ...
      function toggleRadios() {
        if ($('#edit-course-types').hasClass('open')) {
          $('#edit-course-types').removeClass('open');
        }
        else {
          $('#edit-course-types').addClass('open');
        }
        $('form[data-drupal-selector=course-finder-form] #edit-course-types .form-item.form-type-radio:not(.checked)').toggle();
      }
